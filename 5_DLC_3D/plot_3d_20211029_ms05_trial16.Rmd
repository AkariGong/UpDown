...
```{r read data}
filename <- here('20211029_ms05_trial16_DLC_3D.csv')
```
...
data_peaks_1 <- data_peaks %>% 
  subset(frame >= 200 & frame <= 630) # create a subset by matching the range of frame numbers with qualitative analysis
...
max(ccf(data_nojumps_phase$dorsal_lobe, data_nojumps_phase$ventral_lobe, lag = 100, plot = TRUE)[["acf"]])

lagindex <- which.max(ccf(data_nojumps_phase$dorsal_lobe, data_nojumps_phase$ventral_lobe, lag = 100, plot = FALSE)[["acf"]])
...
ifelse(file.exists("20211029_ms05_trial16_3dtail.csv"), yes = askYesNo("Do you want to continue? Check file name!"), no = write_csv(data_nojumps_plot, "20211029_ms05_trial16_3dtail.csv"))
```
