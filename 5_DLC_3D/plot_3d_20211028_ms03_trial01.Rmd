...
```{r}
filename <- here('20211028_ms03_trial01_DLC_3D.csv')
```
...
data_peaks <- data_frame(lobe = "dorsal",
  ...
  add_row(lobe = "ventral",
          frame = data_nojumps_plot$frame[ggpmisc:::find_peaks(-data_nojumps_plot$ventral_lobe)],
          position = data_nojumps_plot$ventrall_lobe[ggpmisc:::find_peaks(-data_nojumps_plot$ventral_lobe)]) 
data_peaks <- data_peaks[order(data_peaks$frame),] # sort all rows by the frame number
data_peaks_1 <- data_peaks %>% 
  subset(frame >= 800 & frame <= 960) # create a subset by matching the range of frame numbers with qualitative analysis
...
ifelse(file.exists("20211028_ms03_trial01_3dtail.csv"), yes = askYesNo("Do you want to continue? Check file name!"), no = write_csv(data_nojumps_plot, "20211028_ms03_trial01_3dtail.csv"))
```
