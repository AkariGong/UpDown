...
params:
  filename: 20220729_ms06_trial11_DLC_resnet50_UpAndDownRear4Feb10shuffle1_500000.csv
  datadir: Data/5_Raw3D
  outputdatadir: Data/6_Processed3D
---
...
```{r}
source(here('Code', '5_DLC_3D','fin_processing_3d.R'))
```

```{r read data}
# Remember to change the file name!
filename <- here(params$datadir, params$filename)
```
...
```{r}
base_name <- str_extract(params$filename, "(\\d{8}_)([^_]+)_([^_]+)")
output_data_file <- here(params$outputdatadir, paste0(base_name, '_3dtail.csv'))
output_data_file
```
...
ifelse(file.exists(output_data_file), 
       yes = askYesNo("Do you want to continue? Check file name!"), 
       no = write_csv(data_nojumps_plot, output_data_file))
```
