...
get_tail_beats <- function(df, var) {
  d <- pull(df, {{var}})
  good <- !is.na(d)
  a <- detect_index(d, ~!is.na(.x))
  
  dd <- (lead(d[good]) - lag(d[good])) / 2
...
for (file1 in datafiles) {
  ...
  data1 <- data1 |> 
    ...
    mutate(across(c(dorsal_top_p_top, dorsal_middle_p_top),
                  list(ming = ~filter_span(bf, .x))))
  ...
  fig2 <-
    ...
    add_lines(data = data1,
              x = ~frame, y = ~dorsal_top_p_top, name = 'dorsal top') |>
    add_lines(data = data1,
              x = ~frame, y = ~ventral_bottom_p_bottom, name = 'ventral bottom')
