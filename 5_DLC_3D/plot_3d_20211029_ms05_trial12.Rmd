...
```{r read data}
filename <- here('20211029_ms05_trial12_DLC_3D.csv')
```
...
data_nojumps %>%
  ...
  #layout(xaxis = list(range = list(150, 480)))
...
data_peaks_1 <- data_peaks %>% 
  subset(frame >= 150 & frame <= 480) # create a subset by matching the range of frame numbers with qualitative analysis
...
```{r obtain phase lag}
df<-na.omit(data_nojumps_plot)

ccf(df$dorsal_lobe, df$ventral_lobe, lag = 80, pl = F)

max(abs(ccf(df$dorsal_lobe, df$ventral_lobe, lag = 100, plot = TRUE)[["acf"]]))

lagindex <- which.max(abs(ccf(df$dorsal_lobe, df$ventral_lobe, lag = 100, plot = FALSE)[["acf"]]))
bestlag <- ccf(df$dorsal_lobe, df$ventral_lobe, lag =100, plot = FALSE)[["lag"]][[lagindex]] %>% print()
```
...
ifelse(file.exists("20211029_ms05_trial12_3dtail.csv"), yes = askYesNo("Do you want to continue? Check file name!"), no = write_csv(data_nojumps_plot, "20211029_ms05_trial12_3dtail.csv"))
```
