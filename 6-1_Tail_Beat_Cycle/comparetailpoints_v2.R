...
lateral_angle_csv_files <- list.files(pattern = ".csv", full.names = TRUE, path = "/Users/minggong/Documents/Tytell_Lab/Project2/Data/UpAndDownLateral3/videos/angle_vertvel_data/")
lateral_angle <- lateral_angle_csv_files %>% map_dfr(~read_csv(.), col_names = TRUE)

alldata <- list()

for (i in seq_along(a[1:42])) {
  file1 <- a[i]
...
write_csv(alldata, 'alldata_5points.csv')
