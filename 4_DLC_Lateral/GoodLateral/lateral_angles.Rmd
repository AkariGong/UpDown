...
```{r}
# Change Knit Directory into Project Directory
...
```{r}
# "Users","minggong","Documents","Tytell_Lab","Project2",
datadir <- here("Data","3_3D_Training", "UpAndDownGoodLateral")
datadir
```
...
```{r}
all_videos <- list.files(path = datadir, pattern = "*.csv")
all_videos_df <- data.frame(all_videos)
```
...
64:76 20220707_ms02
77:88 20220708_ms02
89:105 20220728_ms06
106:115 20220729_ms06

```{r read all data files, message=FALSE, show_col_types = FALSE}
# filter specific date and fish
data <-
  all_videos[grepl("20211014_ms04.", all_videos)] %>%
  map_dfr(~read_lateral_data(.)) %>%
  mutate(date = ymd(date))
```

```{r}
data %>% select(date, fish, trial) %>% unique()
```
...
trial_info_v2$date <- as.Date(trial_info_v2$date, format = "%m_%d_%Y")
...
data <- left_join(data, trial_info_v2, by = c("fish", "date", "trial"))
```
...
```{r}
# Check whether ID, date, fish, and trial match with each other.
# The ID here could be different from the row number in all_videos_df, because trial_info_v2 includes more trials.
data %>% select(ID, date, fish, trial, vertical_direction) %>% unique()
```
...
```{r}
# Some trials have error for the spiny_dorsal_ant_base column.
# Filter out the column for those trials.
datalong <- datalong %>% filter(point != "spiny_dorsal_ant_base")
```
...
body_angle_1 <- points %>% 
  select(ID, date, fish, trial, vertical_direction, frame, time, eye_strip_angle_deg)
...
body_angle_1 <- body_angle_1 %>%
  ...
  mutate(unwrap_angle = unwrap(eye_strip_angle_deg*pi/180)*180/pi,
         original_angle_smooth = EndEffect(butterfilt, unwrap_angle),
...
ggplot(body_angle_1, aes(x = time, y = original_angle_smooth, color = vertical_direction)) +
  geom_point() +
  facet_wrap(~ID, scales = "free") +
  labs(x = "Time (sec)", y = "Original Body angle (eye-strip) (deg)")
```
...
ggplot(body_angle_1, aes(x = time)) +
  geom_point(aes(y = ang_vel_rads, color = 'ang_vel_rads')) +
  geom_point(aes(y = ang_vel_smooth_rads, color = 'smooth')) +
  facet_wrap(~ID, scales = "free") +
...
fish_length <- points %>% 
  group_by(ID) %>% 
  dplyr::filter(frame > 300 & frame < 500) %>%  # This value may varies among different sets of data.
  mutate(fish_length = mean(abs(left_eye_x - peduncle_middle_x), na.rm = TRUE)) %>% 
  ungroup() %>% 
  select(ID, date, fish, trial, fish_length) %>% 
  unique() %>% 
  mutate(mean_fish_length = mean(fish_length, na.rm = TRUE))

speed <- points %>% 
  group_by(ID) %>%
  mutate(left_eye_vx = (lead(left_eye_x) - lag(left_eye_x))/(2*0.005),
         left_eye_vx_BL = left_eye_vx/fish_length$mean_fish_length[1],  # calculate the ratio with the mean_fish_length
         left_eye_vy = (lead(left_eye_y) - lag(left_eye_y))/(2*0.005),
         left_eye_vy_BL = left_eye_vy/fish_length$mean_fish_length[1])# %>% 
...
output_data_file <- here("Data","3_3D_Training", "UpAndDownGoodLateral", "angle_vel_data", "lateral_angle_20211014_ms04_test.csv")
output_data_file
```

```{r save file}
write_csv(x = lateral_angle, path = output_data_file)
```
