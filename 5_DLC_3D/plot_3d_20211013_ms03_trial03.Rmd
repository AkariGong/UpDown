...
```{r}
filename <- here('20211013_ms03_trial03_DLC_3D.csv')
```
...
data_peaks <- tibble(lobe = "dorsal",
  ...
  add_row(lobe = "ventral",
          frame = data_nojumps_plot$frame[ggpmisc:::find_peaks(-data_nojumps_plot$ventral_lobe)],
          position = data_nojumps_plot$ventral_lobe[ggpmisc:::find_peaks(-data_nojumps_plot$ventral_lobe)]) 
data_peaks <- data_peaks[order(data_peaks$frame),] # order all rows by the frame number

data_peaks_1 <- filter(data_peaks, frame >= 170 & frame <= 490) # matching the range of frame numbers with qualitative analysis
# frame number == 172, 173, 219, 230, 275, 300, 326, 367, 400, 406, 430, 438

data_peaks_1 <- data_peaks_1[-c(3, 4, 6, 7, 12, 13, 14, 15, 21, 22, 23),] # create a subset and remove certain rows if needed. The final data frame should have 12 rows, which means 3 tail beats.
...
write_csv(data_nojumps_plot, "ID3_3d.csv")
```

```{r}
plot_ly(data = data_nojumps_plot, x = ~frame) %>%
...
```{r}
# try another method to calculate the "asymmetry"
data_nojumps_plot <- data_nojumps_plot %>% 
  mutate(asym = (dorsal_top_p_top+dorsal_middle_p_top)-(ventral_middle_p_bottom+ventral_bottom_p_bottom))
```
...
plot_ly(data = data_nojumps_plot, x = ~frame) %>%
  ...
  add_markers(y = ~ventral_middle_p_bottom, name = "ventral_middle_p_bottom") %>% 
  add_markers(y = ~ventral_bottom_p_bottom, name = "ventral_bottom_p_bottom") %>% 
  add_markers(y = ~asym, name = "asymmetry")
```
